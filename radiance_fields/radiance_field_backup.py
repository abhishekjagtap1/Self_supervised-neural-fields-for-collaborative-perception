...
class RadianceField(nn.Module):
    ...
    def forward(
        ...
        if self.dynamic_xyz_encoder is not None and has_timestamps:
            ...
            directiion_vis = torch.from_numpy(np.stack(directions.detach().cpu()))
            pos_vis = torch.from_numpy(np.stack(positions.detach().cpu()))

            # Visualize predicted image
            axes[0].imshow(directiion_vis)
            axes[0].set_title('Direction Image')
            axes[0].axis('off')

            # Visualize ground truth image
            axes[1].imshow(pos_vis)
            axes[1].set_title('position Image')
            axes[1].axis('off')
    ...
    def query_rgb(
        ...
        import matplotlib.pyplot as plt
        import numpy as np
        # Choose the number of feature maps to visualize
        geo_features = torch.from_numpy(np.stack(rgb.detach().cpu()))
        pos_vis = torch.from_numpy(np.stack(dynamic_geo_feats.detach().cpu()))
        num_feature_maps = 16
        subset_feature_maps = geo_features[:num_feature_maps]
        ...
        # Loop through the subset of feature maps and plot them
        for i in range(rows):
            for j in range(cols):
                index = i * cols + j
                if index < num_feature_maps:
                    axes[i, j].imshow(subset_feature_maps[index], cmap='viridis')
                    axes[i, j].set_title(f'Feature Map {index}')
                    axes[i, j].axis('off')
